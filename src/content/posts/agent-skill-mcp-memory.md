...
Agent 的记忆体系可以分为四层。前三层借鉴了认知心理学对人类记忆的分类，第四层是 AI 特有的概念，人类没有完全对应的形式。
...
一种常见的实现方式是向量数据库 + SQLite 混合存储，检索时综合考虑语义相似度和时间近因性。

**语义记忆（Semantic Memory）**

存储抽象的知识和规则，不依附于具体事件。"这个项目用 pnpm 不用 npm"、"用户偏好简短回复"——这类稳定的事实和偏好存在语义记忆里。

一种常见的实现方式是图数据库 + 向量存储，支持复杂的关系推理。

**程序性记忆（Procedural Memory）**

认知心理学里的程序性记忆，是"怎么做某件事"的记忆——骑车、打字这类技能，不一定能用语言描述，但身体记住了。

在 AI Agent 里，这一层对应的是**模型权重本身**。那些通过预训练和微调内化进参数的知识、语言能力、推理模式，不在任何外部存储里，每次推理都隐含在内，不需要检索。这是 AI Agent 独有的记忆载体，人类没有完全等价的机制。

> 前三层都是"外部存储→检索→注入上下文"的路径，程序性记忆是唯一一层完全不经过上下文窗口、直接作用于推理过程的记忆形式。
